...
export default {
  ...
  methods: {
    ...
    createSellrestaurant(lng, lat, name, location, food) {
      // 新增座標點，並將地點資訊加到對應的marker上
      return new Feature({
        geometry: new Point(
          transform([lat, lng], "EPSG:4326", "EPSG:3857")
        ), //如果是transform[lng,lat]，座標會無法出現
        name: name,
        location: location,
        food: food,
      });
          ...
          this.markerLayer
            ...
            .addFeature(
              this.createSellrestaurant(
                ...
                this.restaurants[i].name,
                this.restaurants[i].location,
                this.restaurants[i].food
              )
            );
        }
      } catch (error) {
